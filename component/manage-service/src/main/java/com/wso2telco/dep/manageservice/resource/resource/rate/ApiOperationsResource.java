...
@Path("/rate/apioperations/{api}")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class ApiOperationsResource extends AbstractResource {

    @Override
    protected ServiceTypes getService() {
        return ServiceTypes.RATE_API_OPERATIONS;
    }

    @GET
    public Response getApiOperations(@HeaderParam("authorization") String authHeader, @PathParam("api") String api) {
        List<String> pathParamStringList = new ArrayList<>();
        pathParamStringList.add(api);
